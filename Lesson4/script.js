...
class Item {
    constructor(el, img = 'https://picsum.photos/200/300') {
...
/**
 * Наследуется от базовых классов
 */
class ProductsList extends List {
    ...
    _init(){
        ...
        document.querySelector('.search-form').addEventListener('submit', e => {
            e.preventDefault();
            this.filtered(document.querySelector('.search-field').value);
        });
    }
}
...
class Cart extends List {
   ...
   */
    addProduct(element){
        this.getJson(`${API}/addBasket.json`)
            .then(data => {
                if (data.result === 1){
